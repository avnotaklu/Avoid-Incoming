...
SDL_Rect playerStartingRect = {490,450,256,256};
SDL_Rect backgroundStartingRect = {0,0,1000,900};
...
bool media(Entity &player, Entity &background){
    ...
    bool success = true;
    background.load("../../graphics/TwoLaneRoadDwarf.png",backgroundStartingRect,SDL_Rect{0,0,0,0});
    SDL_Surface *gRestartScreenSurface = loadSurface("../../graphics/restart_screen.png");
    gRestartTexture = SDL_CreateTextureFromSurface(renderer,gRestartScreenSurface);

    if(background.mSurface == NULL){
        std::cout<<"loading of image failed"<<std::endl;
        success = false;
    }
    SDL_Rect playerCollisionRect = {80,20,90,220} ;
    player.load("../../graphics/Car.png",SDL_Rect{450,450,256,256},playerCollisionRect);
